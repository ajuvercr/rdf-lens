...
const ShaclSequencePath: BasicLens<Cont, BasicLensM<Cont, Cont>> =
  new BasicLens((c) => {
...
const ShaclAlternativepath: BasicLens<Cont, BasicLensM<Cont, Cont>> =
  new BasicLens((c) => {
    const options = pred(SHACL.alternativePath).one().then(RdfList).execute(
      c,
    );
...
const ShaclPredicatePath: BasicLens<Cont, BasicLensM<Cont, Cont>> =
  new BasicLens((c) => {
    return pred(c.id);
  });
...
> = new BasicLens((c) => {
  const [path, nameCont] = pred(SHACL.path).one().then(ShaclPath).and(
...
const ImpTargetClass: BasicLens<Cont<Term>, BasicLensM<Quad[], Cont<Term>>> =
  ...
  pred(RDF.type).filter(({ id }) => id.equals(RDFS.Class)).expectOne().and(
    empty<Cont<Term>>(),
  )
    .map(([_clazz, start]) =>
...
const TargetSubjectsOf: BasicLens<Cont<Term>, BasicLensM<Quad[], Cont<Term>>> =
  pred(SHACL.targetSubjectsOf).expectOne().map((pred) =>
    match(undefined, pred.id, undefined).thenAll(subject)
  );

const TargetObjectsOf: BasicLens<Cont<Term>, BasicLensM<Quad[], Cont<Term>>> =
  pred(SHACL.targetObjectsOf).expectOne().map((pred) =>
    match(undefined, pred.id, undefined).thenAll(object)
  );

const ShaclTargets: BasicLens<Cont<Term>, BasicLensM<Quad[], Cont<Term>>> =
  new BasicLens((c) => {
...
const ShaclExtract: BasicLensM<Quad[], BasicLensM<Quad[], Obj>> = match(
  ...
  RDF.type,
  SHACL.NodeShape,
).thenAll(subject).mapAll(
  (subj) => {
    // Follow property to extract all shacl properties
    const [fieldLenses, target] = pred(SHACL.property).thenAll(ShaclProperty)
      .and(ShaclTargets).execute(
        subj,
      );

    // These properties are 'and' together
    const fieldsLens = fieldLenses[0].and(...fieldLenses.slice(1)).asMulti();
    // These properties are getting extracted from all subjects
    const fromSub = target.thenSome(
      fieldsLens.and(empty<Cont>()),
    );
